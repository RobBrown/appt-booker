...
import { logger, metrics, withSpan } from "@robbrown/observability-core";
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    if (Array.isArray(additionalAttendees) && additionalAttendees.length > 20) {
      return NextResponse.json({ error: "Too many attendees (max 20)." }, { status: 400 });
    }
    ...
    posthog.capture({
      ...
      properties: {
        ...
        timezone,
        additional_attendees_count: additionalAttendees.length,
        source: "api",
      },
    });
